...
// rwCond fixes rune widths independent of the process locale, so East-Asian
// "ambiguous" glyphs don't silently become double-width on some machines.
var rwCond = &runewidth.Condition{EastAsianWidth: false}
...
func (t *State) parse(c rune) {
	...
	// Width-aware placement: a double-width glyph (CJK, emoji, wide Nerd-Font
	// icon) occupies two columns. Store the rune in the leading cell and mark
	// the trailing cell as a placeholder so one grid column maps to one terminal
	// column without later text drifting.
	w := rwCond.RuneWidth(c)
	if w < 1 {
		w = 1
	}
	if w > 2 {
	...
	// A wide glyph that can't fit before the right margin wraps first.
	if w == 2 && t.cur.X+1 >= t.cols {
		if t.mode&ModeWrap != 0 && t.cols >= 2 {
			t.lines[t.cur.Y][t.cur.X].Mode |= attrWrap
			t.newline(true)
		} else {
			w = 1
	...
	if w == 2 && t.cur.X+1 < t.cols {
		...
		t.lines[t.cur.Y][t.cur.X+1].Mode |= attrWdummy
	}
	if t.cur.X+w < t.cols {
