...
const (
	...
	attrBlink
	attrWrap
	// attrWdummy marks the trailing column of a double-width glyph. The leading
	// cell holds the rune; this placeholder renders as nothing so callers can
	// keep one grid column per terminal column without the glyph's second half
	// pushing later text out of alignment.
	attrWdummy
)
...
// WideDummy reports whether this cell is the trailing placeholder of a
// double-width glyph (its leading cell, one column to the left, holds the rune).
// Renderers should emit nothing for such cells.
func (g Glyph) WideDummy() bool {
	return g.Mode&attrWdummy != 0
}
